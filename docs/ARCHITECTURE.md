...
- **Reflection loops.** A `CritiqueStep` declares `critiques_step_index` and `max_attempts`. On fail with budget remaining, the orchestrator rewinds `current_step` to the critiqued step and writes a retry attempt (`retry_of_action_id` chains back to the prior failed attempt). Budget exhausted → workflow `failed`. Critic agents are first-class entries in the `agents` table — they write their own `step_kind=critique` action rows, never appear in the inbox, and can be invoked from any chain. The Outreach chain uses `voice-critic` and `accuracy-critic`; the voice profile is loaded at runtime from a `preference` memory tagged `metadata.kind=voice_profile`.
- **Conditional steps (`skip_if`).** Each step can declare a `skip_if(StepContext) -> bool` predicate. When it returns True, the orchestrator advances `current_step` without writing an action row. Used by chains that branch on a prior step's result — e.g. rev rec's "configure incomplete projects" checkpoint and "write entries" execution share one chain; one is skipped per run.
- **Checkpoint side effects (`on_approve`).** `CheckpointStep` accepts an optional `on_approve` callback that runs when the human approves the checkpoint. Used by rev rec to requeue a fresh validation cycle when the human signals they've fixed external data.
- **Legacy approval flow** (action whose workflow has no `pattern`) is unchanged: approve → execute synchronously → complete.

## v2 Orchestrator (LangGraph) — in-flight migration

Lives in `app/orchestrator_v2/`, parallel to the v1 prompt-chain orchestrator. The migration plan is `.agent/plans/3.langgraph-multi-agent-rearchitecture.md`. Both orchestrators run side-by-side until Phase 5.

- **Graphs.** Each workflow kind is a `StateGraph` in `app/orchestrator_v2/graphs/{kind}.py` exporting `build_graph() -> GraphSpec`. Approval gates are declared via `interrupt_before=("node_name",)` on the GraphSpec.
...
- **Audit-event taxonomy.** Constants in `app/orchestrator_v2/events.py` cover workflow lifecycle, node entry/exit, approval transitions, agent invocations, and sub-workflow spawn/complete. Call sites must import these constants — no string literals.
...
- **Multi-agent primitives.** `invoke_agent(slug, input, ctx)` in `app/orchestrator_v2/agent_invoke.py` is the uniform agent entry point usable from any node, chat, or sub-agent. `spawn_workflow(kind, input, parent_workflow_id=...)` in `app/orchestrator_v2/spawn.py` starts a child workflow linked to the parent for nested traces.
- **Loop pattern (iterate on human input).** A node may have an outgoing edge that points back to an earlier node, with an interrupt gate sitting on the loop edge. The canonical example is `rev_rec_monthly`: `validate_and_sync` → (incomplete) → `propose_configure` → [interrupt_before `apply_configure_or_loop`] → human approves → `apply_configure_or_loop` → loops back to `validate_and_sync`. One workflow_id covers the full iteration cycle, replacing v1's split-workflow `on_approve = create_workflow + resume` idiom. The graph author owns loop-termination logic; the framework imposes no infinite-loop guard.
...
- **Migrated.** `content_publish` (Phase 1), `rev_rec_monthly` (Phase 2), `outreach_chain` and `content_creation` (Phase 3). All v1 chains have been moved off the bespoke orchestrator; Phases 4–5 cover multi-agent primitives and v1 decommissioning, not chain migration.
- **Provider dispatch.** `invoke_agent` is currently Anthropic-only. The outreach graph (Anthropic) uses it. The content_creation graph (OpenAI) calls `call_openai` directly until Phase 4's provider-aware refactor lands.
- **Multi-agent messaging.** `app/services/agent_messages.py` (backed by the `agent_messages` table) records every agent-to-agent turn with a `thread_id` correlation key and an optional `workflow_id` link. The `ask_agent` tool (`app/tools/agent_tools.py`) is the canonical primitive: it writes the outbound prompt, invokes the target agent via `invoke_agent` (single-turn), writes the inbound reply, and returns `{answer, thread_id}`. `ToolContext` carries `workflow_id` so node-driven `ask_agent` calls automatically link their messages to the workflow. Phase 4 records threads but does not yet *use* prior turns as conversational context — receivers see only the current question. The `_multi_agent_demo` graph (`app/orchestrator_v2/graphs/_multi_agent_demo.py`) is the canonical end-to-end example; it's not registered in production startup, just like `_critique_poc`.

## Approval Inbox Surface (transitional)

During Phases 1–4 the inbox UI dual-sources from two backends: `/actions` (v1 chains still on the bespoke orchestrator) and `/approvals` (v2 graphs). The frontend type `InboxItem = Action | Approval` is discriminated by the presence of `node_name` (`isApproval()`). `getInboxItems()` runs both queries in parallel and merges by `created_at` desc. Phase 5 retires the dual-source when v1 is gone.
