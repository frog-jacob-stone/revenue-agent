      ...
      <aside className="w-64 flex-shrink-0 border-r border-slate-200 flex flex-col">
        <div className="flex items-center justify-between px-3 py-3 border-b border-slate-200">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded text-xs text-cyan-600 hover:text-cyan-700 hover:bg-cyan-500/10 transition-colors disabled:opacity-50"
            title="New chat"
          >
        ...
        {sessionId ? (
          ...
          <div className="flex flex-col h-full items-center justify-center text-slate-500 text-sm gap-3">
            ...
            <button
              onClick={() => createMutation.mutate()}
              disabled={createMutation.isPending}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-cyan-600 bg-cyan-500/10 border border-cyan-500/40 hover:bg-cyan-500/20 disabled:opacity-50"
            >
              <Plus className="w-3.5 h-3.5" />
              Start a new chat
            </button>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-rose-600 transition-opacity p-1"
        title="Delete chat"
      >
