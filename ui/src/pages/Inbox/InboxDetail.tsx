  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
          ...
          <span
            className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${
              isPending
                ...
                ? 'bg-red-400/15 text-red-600 border border-red-400/40'
        ...
        <h1 className="text-lg font-semibold text-slate-900">{action.summary}</h1>
        {action.reasoning && (
          <p className="text-sm text-slate-600 leading-relaxed border-l-2 border-slate-300 pl-3">
            {action.reasoning}
          </p>
        )}
        <p className="text-xs text-slate-400">{fmt(action.created_at)}</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          {isModified && (
            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide bg-amber-400/15 text-amber-600 border border-amber-400/40">
              Modified
            </span>
          )}
          <button
        ...
        <pre className="text-xs text-emerald-700 bg-slate-50 rounded-lg p-4 overflow-x-auto leading-relaxed font-mono">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-emerald-500/15 text-emerald-600 border border-emerald-500/40 hover:bg-emerald-500/25 disabled:opacity-40 transition-colors"
            >
              {busy ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-red-500/15 text-red-600 border border-red-500/40 hover:bg-red-500/25 disabled:opacity-40 transition-colors"
            >
          ...
          {showReject && (
            <div className="border-t border-slate-200 pt-4">
              ...
              <textarea
                className="w-full bg-slate-100 border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-700 placeholder-slate-400 resize-none focus:outline-none focus:border-slate-300"
                rows={3}
                value={rejectReason}
