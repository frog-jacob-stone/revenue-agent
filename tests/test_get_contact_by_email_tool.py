...
async def test_returns_contact_with_primary_company_id():
    ...
    with p_search, p_company:
        result = await _get_contact_by_email(_CTX, email_address="lead@example.com")
...
async def test_returns_not_found_when_contact_missing():
    ...
    with p_search, p_company:
        result = await _get_contact_by_email(_CTX, email_address="nobody@example.com")
...
async def test_invalid_email_returns_error_without_calling_hubspot():
    with patch(
        ...
        result = await _get_contact_by_email(_CTX, email_address="not-an-email")
