...
- [ ] Project-completion tracking — still nothing. Harvest's `is_active` is the only notion of "closed", and it is Harvest's flag, not one this system owns. Two columns the mockup had are deliberately absent from the live tab: **committed end** (Harvest's `ends_on` is editable and moves when a project slips, so it is the current plan, not a commitment) and **projected end** (needs forecasting). Both need a project record this system owns
